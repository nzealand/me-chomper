...
"#.....#.......#.....#",
"#####################"];
let grid,player,spirits,score,lives,state,last,touchStart,powerUntil;
...
function startGame(){
  ...
  grid=MAP.map(r=>r.split(''));score=0;lives=3;state='playing';last=performance.now();powerUntil=0;
...
function spiritThink(s){
  ...
  const fleeing=performance.now()<powerUntil&&!s.eaten;
  opts.sort((a,b)=>{const da=Math.hypot(player.x-(s.x+a.x),player.y-(s.y+a.y));const db=Math.hypot(player.x-(s.x+b.x),player.y-(s.y+b.y));return (fleeing?db-da:da-db)+(Math.random()-.5)*.1});s.dir=opts[0]
}
function eatSpirit(s){score+=200;s.eaten=true;s.x=10;s.y=9;s.px=s.x*TILE+17;s.py=s.y*TILE+17;s.dir=dirs.up;beep(880,.12,'sine');updateHud()}
function update(dt){if(state!=='playing')return;const now=performance.now();if(now>=powerUntil)spirits.forEach(s=>s.eaten=false);moveEntity(player,Math.min(3.1,dt*.19),true);if(centered(player)){const cell=grid[player.y][player.x];if(cell==='.'||cell==='o'){grid[player.y][player.x]=' ';score+=cell==='o'?50:10;if(cell==='o'){powerUntil=now+POWER_TIME;spirits.forEach(s=>s.eaten=false)}beep(cell==='o'?650:470);updateHud();if(!grid.some(r=>r.includes('.')||r.includes('o')))finish(true)}}spirits.forEach(s=>{spiritThink(s);moveEntity(s,Math.min(2.15,dt*(now<powerUntil ? .1 : .13)));if(Math.hypot(s.px-player.px,s.py-player.py)<22){if(now<powerUntil){if(!s.eaten)eatSpirit(s)}else die()}})}
function die(){if(state!=='playing')return;state='dying';powerUntil=0;player.death=performance.now();beep(110,.5,'sawtooth');setTimeout(()=>{lives--;updateHud();if(lives<=0)finish(false);else{player.x=10;player.y=15;player.px=357;player.py=527;player.dir=dirs.left;player.next=dirs.left;spirits.forEach((s,i)=>{s.x=9+i;s.y=9;s.px=s.x*TILE+17;s.py=323;s.eaten=false});state='playing'}},1100)}
...
function drawSpirit(s){ctx.save();ctx.translate(s.px,s.py);const now=performance.now(),frightened=now<powerUntil&&!s.eaten,flashing=powerUntil-now<1800&&Math.floor(now/180)%2===0;const bob=Math.sin(now/160+s.mood)*2;ctx.translate(0,bob);ctx.fillStyle=frightened?(flashing?'#fff7dc':'#3158c9'):s.color;ctx.beginPath();ctx.arc(0,-4,14,Math.PI,0);ctx.lineTo(14,12);ctx.lineTo(7,8);ctx.lineTo(0,13);ctx.lineTo(-7,8);ctx.lineTo(-14,12);ctx.closePath();ctx.fill();ctx.strokeStyle='#100d2c';ctx.lineWidth=2;ctx.stroke();ctx.fillStyle='white';ctx.beginPath();ctx.arc(-5,-5,4,0,7);ctx.arc(5,-5,4,0,7);ctx.fill();ctx.fillStyle='#100d2c';ctx.beginPath();ctx.arc(-4+s.dir.x*2,-4+s.dir.y*2,1.7,0,7);ctx.arc(6+s.dir.x*2,-4+s.dir.y*2,1.7,0,7);ctx.fill();ctx.restore()}
